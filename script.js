...
function handleOrientation(event) {
    ...
    // Ограничиваем наклон, чтобы түндүк не "улетал" при резком движении телефона.
    const tiltX = Math.max(-12, Math.min(12, (beta - 45) / 4));
    const tiltY = Math.max(-12, Math.min(12, gamma / 4));
...
function startDeviceOrientation() {
    ...
    if (
        ...
        // Android and browsers that don't require an explicit permission prompt.
        window.addEventListener('deviceorientation', handleOrientation, true);
    }
}
